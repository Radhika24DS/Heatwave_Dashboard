  ...
  return (
    <div
      className={[
        ...
        padMap[padding],
        hover ? 'transition-all duration-300 hover:-translate-y-0.5 hover:border-brand-primary/30 hover:shadow-glow' : '',
        glow ? 'heat-ring border-brand-primary/30' : 'border-brand-border/60',
        className,
      ].filter(Boolean).join(' ')}
    >
      {/* Corner background glow */}
      <div className={`absolute -top-12 -right-12 w-28 h-28 rounded-full bg-brand-primary/5 blur-2xl pointer-events-none transition-all duration-500 ${glow ? 'opacity-30 bg-brand-primary/15' : 'opacity-10'}`} />
      {title && (
